...
const ProductDetails = () => {
  ...
  if (status.products != null) {
    const clicked_prod = status.products.filter(
      (product) => product.name === status.currProduct.name
    );
    console.log(
      "ProductDetails.js -> estoque do produto: " + clicked_prod[0].stock
    );
  } else {
    console.log(
      "ProductDetails.js -> estoque do produto: não foi possivel pegar"
    );
  }
