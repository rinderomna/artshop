...
Para descrever a implementação das funcionalidades listadas nos requisitos do projeto, utilizamos a ferramenta 'Marvel' para construir um <a href="https://marvelapp.com/prototype/884i1de/screen/91326238?target=_blank)" target="_blank">protótipo da aplicação (mockups das telas do sistema) [Acesse clicando neste texto linkado]</a>. Adicionalmente, a tela principal e duas outras <<decidir quais>> foram desenvolvidas em HTML5 + CSS3, e estão disponíveis no repositório do projeto em <<caminhos>>. Para auxílio da compreensão da proposta de navegação na aplicação, foi desenhado um [diagrama de navegação](images&diagrams/DiagramadeNavegacao_Jaguar.pdf).
...
* Um primeiro comentário é sobre o código feito para as telas de mockup. O protótipo com as telas do sistema foi feito na ferramenta Marvel para que, junto do diagrama de navegação, auxiliasse na implementação em código destas. No entanto, há duas limitações iniciais que impedem que estas telas sejam altamente semelhantes ao protótipo: limitações na capacidade dos desenvolvedores de refletir o design completamente em código; e limitações iniciais no conhecimento de tecnologias de desenvolvimento Web, como frameworks (e.g. React, Angular, Vue), os quais serão estudados em etapas posteriores da disciplina. Portanto, este comentário visa deixar claro que as telas implementadas em código não serão idênticas aos protótipo de design, e que, inicialmente, ainda não possuirão as mesmas capacidades que terão em sua implementação final, após todo o conteúdo da disciplina ter sido estudado. No entanto, desde o início, elas ja irão refletir as propostas feitas para o sistema, detalhadas nos requisitos e na descrição do projeto.
* Um segundo comentário sobre as telas de mockup no protótipo do Marvel, é que as telas referentes à opção 'Camisetas' na navbar não foram desenvolvidas, pois seriam análogas às referentes a 'Prints' e 'Adesivos'. As telas para estes dois últimos tipos de produtos já são suficientemente ilustrativas para representar o sistema no protótipo.
* Um último comentário sobre os mockups e o diagrama de navegação dessenvolvidos é que eles servem como documentação do projeto e base para o desenvolvimento. Contudo, não esperamos que o desenvolvimento fique amarrado a eles. Logo, podemos no decorrer do projeto, realizar dinamicamente alterações e complementos ao sistema.
