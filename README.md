...
* Além dos avisos do console do navegador, há alguns do compilador *React* (aparecem ao executar 'npm start' no terminal). Eles se relacionam, em sua maioria, ao 'es-lint'. 
  * Alguns deles foram ignorados com um comentário específico para desabilitar o 'es-lint' em uma linha de código, pois, ao tentar resolvê-los, a aplicação não se comportava da maneira desejada, e não resolvê-los não parecia comprometer o seu desempenho (precisamos desabilitar o 'es-lint' em uma parte da barra de navegação, por exemplo, para obter o efeito de destaque laranja que pretendíamos - o 'es-lint' pedia dependências adicionais no *useEffect* que comprometiam a boa usabilidade da barra de navegação).
