...
Entrando as informações de login como cliente na tela de login e clicando em 'entrar', o cliente irá para a Homepage do site, que é muito semelhante à Homepage que tinha disponível no contexto deslogado, com a listagem de todos os produtos e as opções de navegação na navbar. Porém, agora, há duas funcionalidades a mais: o ícone de usuário e o ícone de carrinho. Clicando no ícone de carrinho, a aba lateral de carrinho é mostrada com os itens que foram adicionados até então. Clicando no ícone de usuário, é aberto um menu lateral com as seguintes opções: editar perfil, meus pedidos e sair. Selecionando 'editar perfil', o cliente irá para uma tela em que pode alterar sus informações. Em 'meus pedidos', o cliente obtém uma tela com uma listagem dos pedidos que já fez, com seus status 'pendente' ou 'enviado' (caso em que tem a transportadora e o código de envio para acompanhar a entrega no sistema externo da transportadora). Clicando em 'sair' no menu lateral de cliente, o cliente é deslogado do sistema e levado para a Homepage em contexto 'deslogado'.
...
* Um primeiro comentário é sobre o código feito para as telas de Mockup. O protótipo com as telas do sistema foi feito na ferramenta Marvel para que, junto do diagrama de navegação, auxiliasse na implementação em código das telas do sistema. No entanto, há duas limitações iniciais que impedem que estas telas sejam altamente semelhantes ao protótipo: limitações na capacidade dos desenvolvedores de refletir o design completamente em código; e limitações iniciais no conhecimento de tecnologias de desenvolvimento Web, como frameworks (e.g. React, Angular, Vue), os quais serão estudados em etapas posteriores da disciplina. Portanto, este comentário visa deixar claro que as telas implementadas em codigo não serão idênticas aos protótipo de design, e que, inicialmente, ainda não possuirão as mesmas capacidade que terão em sua implementação final, após todo conteúdo da disciplina ter sido estudado. No entanto, desde o início, elas ja irão refletir as propostas feitas para o sistema, detalhadas nos requisitos e na descrição do projeto.
