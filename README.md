...
Para descrever a implementação das funcionalidades descritas nos requisitos do projeto, utilizamos a ferramenta 'Marvel' para construir um protótipo da aplicação (mockups das telas do sistema). Adicionalmente, a tela principal e duas outras <<decidir quais>> foram desenvolvidas em HTML5 + CSS3, e estão disponíveis no repositório no projeto em <<caminhos>>. Para auxílio da compreensão da proposta de navegação na aplicação, foi desenhado um diagrama de navegação.

O diagrama de navegação possui retêngulos que representam as telas do sistema, que estão organizadas hieraquicamente em contextos. Em relação ao status de login no sistema, há 4 4 contextos distintos:
* Deslogado
* Transiente
* Cliente logado
...
Na tela transiente de login, caso as informações sejam de um administrador, o usuário é levado para a Homepage do site com a visão de um administrador. O administrador pode, nesta Homepage, clicar na opção 'novo item' para cadastrar um novo produto. Na tela de cadatro de produto, o administrador preenche as informações do novo produto, clica em 'ok' e, em caso de cadastro bem-sucedido, é levado de volta à Homepage com visão de administrador. Na Homepage, o administrador também possui uma listagem de todos os produtos, podendo pela navbar também acessar as seções de com filtragens por tipo de produto. Ao clicar em 'ver mais' em algum produto, o administrador é direcionado a uma tela de edição do produto, em que pode alterar suas informações e até excluí-lo. Ao concluir a edição com 'ok', é apresentado ao administrador uma mensagem de confirmação de conclusão da operação de edição, porém ele é mantido na mesma tela. Se ele tiver optado por excluir o produto, ele é direcionado à tela de cadastro de um novo produto.
